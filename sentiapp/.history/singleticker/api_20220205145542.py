...
def get_single_tweet_sentiment(request, ticker_id,  sentiment_type, value_sentiment, date):
    # stock = get_object_or_404(StockSummary, ticker=ticker_id)

    # dictornary  = {
    #     "positive" : "overall_pos",
    #     "negative" : "overall_neu",
    ...
    # }
    # sentiment_type   = dictornary[value_sentiment.lower()]
    # record  =  HourlyRecord.objects.filter(stock =  stock,  sentiment_type =  value_sentiment, tweet_date = 
...
def getSentiment24Hours(request, ticker_id):
    ...
    list_ids = records.values_list('_id', flat=True)
