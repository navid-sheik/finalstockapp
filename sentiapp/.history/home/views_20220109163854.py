...
def index(request):
    ...
    for tweet in cursor:
        ...
        try:
            stockSummary = TweetRecord.objects.get(tweet_id=tweet.id_str)
        except StockSummary.DoesNotExist:
...
def createHourlyRecord(ticker):
    ...
    list_records_previous_day =  TweetRecord.objects.filter(stock = stock,tweet_date__hour__range=(current_hour - 1, current_hour))
